...
遵循IEEE754数值格式的语言的浮点计算会出现精度损耗的通病，`不仅仅是js，其他常见语言如java，c等都存在这个问题。`因此尽量不要进行某个特定浮点数值的测试。

在二进制下，浮点数会出现大量的循环数，如：
```
0.1 -> 0.0001100110011001...(无限)
0.2 -> 0.0011001100110011...(无限)
```
实际上，0.1到0.9，只有0.5能用二进制精确表示。

`解决方案`：使用三方计算库、手动转换成整数计算之后再还原。

### 附：IEEE 754浮点表示标准

`float单精度（32位）浮点数`：在机器中表示用 1 位表示数字的符号，用 8 位来表示指数，用23 位来表示尾数，即小数部分。

`double双精度（64位）浮点数`：用 1 位表示符号，用 11 位表示指数，52 位表示尾数，其中指数域称为阶码。

对于js整数运算，范围是正负2的53次方，对于整数的位运算（比如移位等操作），JavaScript仅支持32位整型数。

### Number.toFixed()四舍五入的改良

js原生的四舍五入函数，各浏览器实现的不同，会出现如`0.15.toFixed(1)=0.1`这种明显错误，有需要的地方可以重写该接口。

## 正则表达式
js支持使用正则表达式来处理字符串。
### 语法
语法参考：https://www.baidufe.com/item/eb10deb92f2c05ca32cf.html
可以使用`元字符`、`反义字符`、`转义字符`、`重复匹配`、`修饰符`等多种组合，来实现对字符串模式的精确控制。
内置`RegExp 对象`，包括`test()`判断有无和`exec()`找到位置两个常用接口。
`String`对象自带`replace()`接口，接受reg参数来精确操作。
* 元字符	对应说明
  * .	匹配除换行符之外的任意字符
...
### $n（n为数字1-9）表达式
`RegExp.$1...$9`属性用于返回正则表达式模式中某个子表达式匹配的文本。`子表达式`是指`每个`小括号内的部分表达式，包括嵌套。`RegExp.$1...$9`属性的值为String类型，返回上一次正则表达式匹配中，第n个子表达式所匹配的文本。
...
str = '<a href="http://www.baidu.com/" title="首页" target="_blank">fiberhome</a>';
// 提取URL
reg = /<a\s[^>]*href="([^">]+)"[^>]*>/;
reg.exec(str);
document.writeln(RegExp.$1); // http://www.baidu.com/
```
